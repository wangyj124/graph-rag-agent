...
from utils.api import clear_chat, sync_communities, clear_cache
...
def display_sidebar():
    ...
    with st.sidebar:
        ...
        # [修改开始]
        # 类型: 修改 | 时间: 2026-02-04
        # 目的: 添加同步社区摘要按钮，解决数据不一致问题
        # ==============================================================================

        st.markdown("---")
        st.header("数据维护")
        if st.button("🔄 同步社区摘要", help="当您手动修改了数据库中的实体属性时，点击此按钮重新生成社区摘要快照。"):
            with st.spinner("正在重新同步社区摘要，这可能需要几十秒到几分钟，请稍候..."):
                result = sync_communities()
                if result.get("success"):
                    st.success(result.get("message", "同步成功！"))
                    # 清除前端缓存以确保下次查询使用最新数据
                    clear_cache()
                    st.info("已自动清除前端缓存。")
                else:
